...
Passwordless.configure do |config|
  ...
  config.expires_at = -> { 10.minutes.from_now } # How long until a signed in session expires.
  ...
  config.success_redirect_path = '/account' # After a user successfully signs in
  config.failure_redirect_path = '/sign-in' # After a sign in fails
